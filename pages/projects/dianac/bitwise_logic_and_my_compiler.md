# Bitwise Logic & My Compiler

> <b style="color: var(--gold)">Warning:</b> I took heavy "inspiration" from the Rust compilers source code, if something looks too smart for 19-year-old me to have come up with that's where it's from...
...
The Diana II is 6-bit minimal instruction set computer designed around using `NOR` as a universal logic gate. `NOR` doesn't allow bit permutations, so I used rotate lookup tables to preform those.
...
This following section (until the next `h2`) is the boring part, however, it's important to help understand the code following. But if you really don't care, you may [skip](#my-compiler-architecture) it.
