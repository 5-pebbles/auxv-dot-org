...
use rsa::{Oaep, RsaPrivateKey, RsaPublicKey, pkcs8::DecodePrivateKey};
...
impl Analytics {
    ...
    fn record_page_view(&self, view: &PageView) {
    ...
    fn decrypt_all(&self, private_key: &RsaPrivateKey) -> Result<Vec<PageView>, String> {
        let connection = self.connection.lock().unwrap();
    ...
    async fn on_response<'r>(&self, request: &'r Request<'_>, response: &mut Response<'r>) {
        ...
        let view = PageView {
            ...
            visitor_hash: request
                .client_ip()
                .map(|ip| hash_visitor(&analytics.visitor_salt, &ip.to_string())),
        };
...
pub fn dashboard(analytics: &rocket::State<Analytics>, form: Form<DecryptForm>) -> RawHtml<String> {
    ...
    match analytics.decrypt_all(&private_key) {
        Ok(views) => RawHtml(render::render_dashboard(&views)),
...
fn hash_visitor(salt: &[u8; 32], client_ip: &str) -> String {
    let day_number = OffsetDateTime::now_utc().unix_timestamp() / 86400;
